...
export default async function handleSync(msg: string) {
  let appData = JSON.parse(msg) as AppData[]

  sendNotification(`Start Syncing...`)
  ...
  try {
    ...
    sendNotification('Failed to update existing notes. Sync was not completed.')
    return
  }
  ...
  try {
    ...
    sendNotification('Failed to delete obsolete notes. Sync was not completed.')
    return
  }
